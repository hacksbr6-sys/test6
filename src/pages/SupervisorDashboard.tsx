...
const SupervisorDashboard: React.FC = () => {
  ...
  // Verificar se o usuário é encarregado
  useEffect(() => {
    if (!currentUser || currentUser.type !== 'mechanic' || currentUser.position !== 'encarregado') {
      navigate('/');
    }
  ...
  const handleAddCar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          type: 'general',
          message: `Novo veículo adicionado por ${currentUser?.full_name}: ${carForm.brand} ${carForm.model} (${carForm.year}) - $${carForm.price.toLocaleString()}`,
          is_read: false
        });
  ...
  // Filtrar notificações relacionadas a vendas de veículos
  const carSaleNotifications = notifications.filter(n => 
    n.type === 'car_sale' || n.message.toLowerCase().includes('veículo') || n.message.toLowerCase().includes('carro')
  );
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      {/* Header */}
      <div className="bg-black/90 backdrop-blur-sm border-b-2 border-green-600">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-white font-bold text-lg">PAINEL DO ENCARREGADO</h1>
                  <p className="text-green-400 text-sm">Gestão de Carros e Notificações</p>
                </div>
              </div>
            </div>
            ...
            <div className="text-right">
              ...
              <p className="text-green-400 text-sm">Encarregado</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex space-x-1 bg-black/50 p-1 rounded-xl mb-8 w-fit">
          ...
          >
            ...
            {carSaleNotifications.filter(n => !n.is_read).length > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                {carSaleNotifications.filter(n => !n.is_read).length}
              </span>
            )}
          </button>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {cars.map((car) => (
                <div
                  key={car.id}
                  className="bg-black/50 rounded-xl border border-gray-800 hover:border-green-600 transition-all duration-300 overflow-hidden"
                >
                  <div className="aspect-w-16 aspect-h-9 relative">
                    ...
                    />
                    <div className="absolute top-4 right-4 bg-green-600 text-white px-3 py-1 rounded-lg font-bold">
        ...
        {activeTab === 'notifications' && (
          ...
          >
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-white">Notificações de Vendas</h2>
              {carSaleNotifications.filter(n => !n.is_read).length > 0 && (
                <button
            ...
            <div className="space-y-4">
              {carSaleNotifications.length === 0 ? (
                <div className="text-center py-12">
                  <Bell className="h-16 w-16 text-gray-600 mx-auto mb-4" />
                  <h3 className="text-xl font-bold text-gray-400 mb-2">
                    Nenhuma notificação de venda
                  </h3>
                  <p className="text-gray-500">
                    As notificações de pedidos de venda aparecerão aqui
                  </p>
                </div>
              ) : (
                carSaleNotifications.map((notification) => (
                  <div
                    key={notification.id}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <AlertCircle className={`h-4 w-4 ${
                            notification.is_read ? 'text-gray-400' : 'text-green-400'
                          }`} />
                          <span className={`text-sm font-medium ${
                            notification.is_read ? 'text-gray-400' : 'text-green-400'
                          }`}>
                            {notification.type === 'car_sale' && 'Pedido de Venda'}
                            {notification.type === 'general' && notification.message.toLowerCase().includes('veículo') && 'Veículo Adicionado'}
                            {notification.type === 'general' && !notification.message.toLowerCase().includes('veículo') && 'Geral'}
      ...
      {showAddCarModal && (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-gray-900 rounded-xl p-6 max-w-md w-full border border-green-600"
          >
